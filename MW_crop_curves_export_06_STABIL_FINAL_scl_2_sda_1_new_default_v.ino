...
<div id='chartsArea'>
  ...
  <!-- horizontale Y-Guides: je Chart Top/Bot -->
...
cbXGuides.addEventListener('change', function(){
  if(isCropped){ setXGuidesVisible(false); return; }
  setXGuidesVisible(cbXGuides.checked);
});
cbYGuides.addEventListener('change', function(){
  if(yIsCropped){ setYGuidesVisible(false); return; }
  ...
  fetch('/status').then(function(r){return r.json();}).then(function(s){
    ...
    if(wasRunning && !running){
      if(posFull.length>1){
        ...
        setXGuidesVisible(cbXGuides.checked && !isCropped);
        setYGuidesVisible(cbYGuides.checked && !yIsCropped);
...
function onDragY(e){
  if(!draggingY || !cbYGuides.checked || yIsCropped) return;
...
function refreshCharts(){
  ...
  setXGuidesVisible(cbXGuides.checked && !isCropped);
  updateGuidePositions();
  if(cbYGuides.checked && !yIsCropped){ updateYGuidePositionsAll(); } else { setYGuidesVisible(false); }
...
function updateYGuidePositionsAll(){
  if(!cbYGuides.checked || yIsCropped) { setYGuidesVisible(false); return; }
  for(var i=0;i<chartDefs.length;i++){
    var d=chartDefs[i];
    if(d.yTop===null || d.yBot===null) continue;
    var topPx = displayValueToPxYFor(d, d.yTop);
    var botPx = displayValueToPxYFor(d, d.yBot);
    d.gTop.style.top = topPx + 'px';
    d.gBot.style.top = botPx + 'px';
  }
